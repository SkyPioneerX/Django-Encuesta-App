    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if check1 or check2 or check3:
                messages.error(
                    request, "Registro fallido!", extra_tags='alert alert-warning alert-dismissible fade show')
                return redirect('accounts:register')
            else:
                user = User.objects.create_user(
                    username=username, password=password1, email=email)
                messages.success(
                    request, f'Gracias por registrarte {user.username}.', extra_tags='alert alert-success alert-dismissible fade show')
